...
public class RoadmapGenerationService {
    ...
    public Map<String, List<String>> generateRoadmap(List<Skill> missingSkills, String level, int hoursPerWeek) {
        ...
        log.info("Dispatching LLM Roadmap Request. Mapping {} missing skills securely.", missingSkills.size());

        // Demo Mode: If the URL is a placeholder, simulate an AI failure to demonstrate the Fallback mechanism.
        if (llmApiUrl.contains("example-llm.com")) {
            log.info("LLM Service in Demo Mode (Simulating Failure). Triggering Fallback Video Roadmap.");
            throw new RoadmapGenerationException("Demo Mode: Explicitly simulating LLM failure to trigger fallback.");
        }
        ...
        try {
            ...
            log.info("LLM Generation Request completed successfully with payload format OK.");
            return parseAndValidateResponse(response.getBody(), skillNames);

        } catch (RestClientException e) {
            log.error("LLM Execution Crash: Network explicitly timed out or failed.", e);
            throw new RoadmapGenerationException("LLM API network failure or timeout", e);
        } catch (Exception e) {
            log.error("LLM Parse Failure: Output bounds violated or unknown schema.", e);
