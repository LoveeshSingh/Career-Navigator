...
import com.skillbridge.career_navigator.dto.ValidatedSkillDto;
...
import com.skillbridge.career_navigator.repository.SkillRepository;
...
public class SkillValidationService {
    ...
    private Optional<Skill> findSkillInDatabase(String term) {
        // Direct Name Match
        Optional<Skill> explicitMatch = skillRepository.findByName(term);
        if (explicitMatch.isPresent()) {
            return explicitMatch;
        }

        // Alias Match
        Optional<SkillAlias> aliasMatch = skillAliasRepository.findByAliasName(term);
